...
- [COSAI-Labs/toolpipe-mcp-server](https://github.com/COSAI-Labs/make-money-30day-challenge/tree/master/products/mcp-server) 📇 ☁️ - 238+ developer utility tools for AI agents. All-in-one MCP server with code review, JSON formatting, PDF tools, QR codes, hashing, UUID generation, DNS lookup, regex testing, JWT decoding, crypto prices, web scraping, SEO analysis, and more. Install via `npx @cosai-labs/toolpipe-mcp-server`.
