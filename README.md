...
- [yamcp](https://github.com/hamidra/yamcp) 🍠 - An MCP workspace manager to bundle and manage MCP servers in dedicated local workspaces (e.g., for coding, design, research).Integrates each workspace into your app as a single stdio MCP gateway. It allows monitoring of all server interactions through a consolidated log store. The tool also scans each workspace to ensure servers are secure and function as expected.
